...
RUNS_ROOT   = f"{BASE}/{CFG['runs_dir']}".rstrip("/")
PSEUDO_ROOT = f"{BASE}/{CFG['pseudo_dir']}".rstrip("/")
...
def run_gen(gen, data_yaml, hist_csv):
    ...
    model.train(**{**TRAIN_ARGS, "data": data_yaml}, project=RUNS_ROOT, name=os.path.basename(run), exist_ok=True)
    ...
    with open(os.path.join(run, "val_metrics.json"), "w") as f:
        json.dump({
            ...
            "val_conf": VAL_ARGS.get("conf", 0.25),
